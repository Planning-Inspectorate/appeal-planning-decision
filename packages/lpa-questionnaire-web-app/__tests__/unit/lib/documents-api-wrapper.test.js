...
describe('lib/documents-api-wrapper', () => {
  ...
  describe('createDocument', () => {
    ...
    it('should throw if the remote API response is not ok', async () => {
      ...
      try {
        await createDocument(mockId, data, null, documentTypes.officersReport.name);
    ...
    it('should return the expected response if the fetch status is 202 with form data input', async () => {
      ...
      expect(await createDocument(mockId, data, null, documentTypes.officersReport.name)).toEqual({
        applicationId: 123,
        id: '123-abc-456-xyz',
        name: 'tmp-2-1607684291243',
      });
    ...
    it('should return the expected response if the fetch status is 202 with form data input with name overrided', async () => {
      ...
      expect(
        await createDocument(mockId, data, 'namePreferred.pdf', documentTypes.officersReport.name)
      ).toEqual({
        applicationId: 123,
        id: '123-abc-456-xyz',
        name: 'namePreferred.pdf',
    ...
    it('should return the expected response if the fetch status is 202 with data buffer input', async () => {
      ...
      expect(
        await createDocument(mockId, Buffer.from('data'), null, documentTypes.officersReport.name)
      ).toEqual({
        applicationId: 123,
        id: '123-abc-456-xyz',
        name: 'tmp-2-1607684291243',
