...
describe('lib/pdf-api-wrapper', () => {
  ...
  describe('generatePDF', () => {
    ...
    it('should throw if fetch rejects htmlContent', async () => {
      fetch.mockReject(new Error('fake error message'));
      expect(generatePDF(mockReply.id, htmlContent)).rejects.toThrow('fake error message');
    });

    it('should throw if the remote API response is not ok', () => {
      fetch.mockResponse('fake response body', { status: 400 });
      expect(generatePDF(mockReply, htmlContent)).rejects.toThrow('Bad Request');
    });

    it('should throw if the response code is anything other than a 202', async () => {
      fetch.mockResponse('a response body', { status: 204 });
      expect(generatePDF(mockReply.id, htmlContent)).rejects.toThrow('No Content');
    });
    ...
    it('should return the expected response if the fetch status is 200', async () => {
      ...
      const result = await generatePDF(mockReply.id, htmlContent);
