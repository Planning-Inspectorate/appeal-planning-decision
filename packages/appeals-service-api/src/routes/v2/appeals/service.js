...
async function getAppealsForUser(userId, role) {
	...
	// drafts
	const { v2Drafts, draftSubmissions } = isAppellantOrAgent
		? await getAppellantDrafts(user)
		: { v2Drafts: [], draftSubmissions: [] };
...
/**
 *
 * @param {import('./repo').UserWithAppeals} user
 */
async function getAppellantDrafts(user) {
	...
	return { v2Drafts, draftSubmissions: draftSubmissions.filter(filterNotNull) };
}
