...
module.exports = ({
	...
	describe('/api/v2/appeal-cases/:caseReference/appellant-statement-submission/submit', () => {
		...
		it.each(appealTypes)(
			'Formats appellant statement submission without docs %s',
			async (caseType) => {
				...
				await appealsApi
					.post(`/api/v2/appeal-cases/${id}/appellant-statement-submission`)
					.send(appellantStatementData);
				await appealsApi
					.post(`/api/v2/appeal-cases/${id}/appellant-statement-submission/submit`)
					.expect(200);
		...
		it.each(appealTypes)(
			'Formats appellant statement submission with docs %s',
			async (caseType) => {
				const id = crypto.randomUUID();
				const statement = {
					caseReference: id,
					representation: `Another statement text for appellant case ${id}`,
					representationSubmittedDate: expect.any(String),
					representationType: 'statement',
					serviceUserId: testServiceUserId,
					documents: [
				...
				const appellantSubmissionData = {
					appellantStatement: `Another statement text for appellant case ${id}`,
