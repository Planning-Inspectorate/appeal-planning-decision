...
module.exports = ({ getSqlClient, setCurrentSub, setCurrentLpa, appealsApi }) => {
	...
	/**
	 * @returns {Promise<import('@pins/database/src/client/client').AppellantStatementSubmission>}
	 */
	const createStatementSubmission = async (caseRef, caseType) => {
		...
		const statement = await sqlClient.appellantStatementSubmission.create({
			data: {
				AppealCase: {
					connect: {
	...
	describe('/api/v2/appeal-cases/:caseReference/appellant-statement-submission/document-upload', () => {
		it.each(appealTypes)('Handles document for case type: %s', async (caseType) => {
			...
			const postResponse = await appealsApi
				.post(`/api/v2/appeal-cases/${caseRef}/appellant-statement-submission/document-upload`)
			...
			const uploadedData = await sqlClient.submissionDocumentUpload.findMany({
				where: {
					appellantStatementId: statementSubmission.id
				}
			});
			...
			const deleteResponse = await appealsApi
				.delete(`/api/v2/appeal-cases/${caseRef}/appellant-statement-submission/document-upload`)
