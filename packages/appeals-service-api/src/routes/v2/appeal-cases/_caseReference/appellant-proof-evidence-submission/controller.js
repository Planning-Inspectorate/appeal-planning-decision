 ...
 */
async function getAppellantProofOfEvidenceSubmission(req, res) {
	try {
		const content = await getAppellantProofOfEvidenceByAppealId(req.params.caseReference);
		if (!content) {
 ...
 * @type {import('express').RequestHandler}
 */
async function createAppellantProofOfEvidenceSubmission(req, res) {
	try {
		const content = await createAppellantProofOfEvidence(req.params.caseReference, req.body);
		if (!content) {
 ...
 * @type {import('express').RequestHandler}
 */
async function patchAppellantProofOfEvidenceSubmission(req, res) {
	try {
		const content = await patchAppellantProofOfEvidenceByAppealId(
