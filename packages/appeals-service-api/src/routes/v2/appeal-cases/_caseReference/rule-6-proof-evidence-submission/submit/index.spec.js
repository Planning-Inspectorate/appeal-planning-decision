...
module.exports = ({
	...
	describe('/api/v2/appeal-cases/:caseReference/rule-6-proof-evidence-submission/submit', () => {
		...
		it.each(appealTypes)(
			'rule 6 proof of evidence submission with one doc type %s',
			async (caseType) => {
				...
				const result = await appealsApi
					.post(`/api/v2/appeal-cases/${id}/rule-6-proof-evidence-submission`)
				...
				await appealsApi
					.post(`/api/v2/appeal-cases/${id}/rule-6-proof-evidence-submission/submit`)
					.expect(200);
		...
		it.each(appealTypes)(
			'rule 6 proof of evidence submission with both doc types %s',
			async (caseType) => {
				const id = crypto.randomUUID();
				const formattedProofs = {
					caseReference: id,
