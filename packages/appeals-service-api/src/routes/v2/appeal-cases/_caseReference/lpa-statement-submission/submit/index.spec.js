...
module.exports = ({
	...
	describe('/api/v2/appeal-cases/:caseReference/lpa-statement-submission/submit', () => {
		...
		it.each(appealTypes)('Formats lpa statement submission without docs %s', async (caseType) => {
			...
			await appealsApi
				.post(`/api/v2/appeal-cases/${id}/lpa-statement-submission`)
				.send(lpaStatementData);
			await appealsApi
				.post(`/api/v2/appeal-cases/${id}/lpa-statement-submission/submit`)
				.expect(200);
		...
		it.each(appealTypes)('Formats lpa statement submission with docs %s', async (caseType) => {
			const id = crypto.randomUUID();

			const statement = {
				caseReference: id,
				representation: `Another statement text for lpa case ${id}`,
				representationSubmittedDate: expect.any(String),
				representationType: 'statement',
				lpaCode: 'Q9999',
				documents: [
			...
			const lpaFinalCommentData = {
				lpaStatement: `Another statement text for lpa case ${id}`,
