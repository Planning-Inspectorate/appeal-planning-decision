...
export class HorizonInteraction {
	...
	static getCreateContactInteraction(
		...
		if (expectation.getOrganisationId()) {
			interaction.addJsonValueExpectation(
				JsonPathExpression.create("$.AddContact.contact['a:OrganisationID']"),
				expectation.getOrganisationId()
			);
		}
	...
	static getCreateAppealInteraction(
		...
		let interaction = new Interaction('Create appeal')
			...
			.addJsonValueExpectation(
				JsonPathExpression.create('$.CreateCase.LPACode'),
				expectation.getLpaCode()
			)
			.addJsonValueExpectation(
			...
			.addJsonValueExpectation(
				JsonPathExpression.create('$.CreateCase.location'),
				expectation.getLpaCountry()
			)
			.addJsonValueExpectation(
	...
	static getCreateDocumentInteraction(
		...
		let interaction = new Interaction('Create document')
			...
			.addJsonValueExpectation(
				JsonPathExpression.create(`${documentData}['a:Content']`),
				new RegExp('^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$')
			) // Base 64 encoded string
			.addJsonValueExpectation(
				JsonPathExpression.create(`${documentData}['a:DocumentType']`),
				/^\w|\s+$/
			) // Words and spaces
			.addJsonValueExpectation(
				JsonPathExpression.create(`${documentData}['a:Filename']`),
				document.name
			)
			.addJsonValueExpectation(
				JsonPathExpression.create(`${documentData}['a:IsPublished']`),
				'false'
			)
			.addJsonValueExpectation(JsonPathExpression.create(`${documentData}['a:NodeId']`), '0')
			.addJsonValueExpectation(
				JsonPathExpression.create(`${documentMetadata}[0]['a:AttributeValue']['__i:type']`),
				'a:StringAttributeValue'
			)
			.addJsonValueExpectation(
				JsonPathExpression.create(`${documentMetadata}[0]['a:AttributeValue']['a:Name']`),
				'Document:Involvement'
			)
			.addJsonValueExpectation(
				JsonPathExpression.create(`${documentMetadata}[0]['a:AttributeValue']['a:Value']`),
				/^Appellant|LPA|$/
			) // The words "Appellant", "LPA", or an empty string
			.addJsonValueExpectation(
				JsonPathExpression.create(`${documentMetadata}[1]['a:AttributeValue']['__i:type']`),
				'a:StringAttributeValue'
			)
			.addJsonValueExpectation(
				JsonPathExpression.create(`${documentMetadata}[1]['a:AttributeValue']['a:Name']`),
				'Document:Document Group Type'
			)
			.addJsonValueExpectation(
				JsonPathExpression.create(`${documentMetadata}[2]['a:AttributeValue']['__i:type']`),
				'a:StringAttributeValue'
			)
			.addJsonValueExpectation(
				JsonPathExpression.create(`${documentMetadata}[2]['a:AttributeValue']['a:Name']`),
				'Document:Incoming/Outgoing/Internal'
			)
			.addJsonValueExpectation(
				JsonPathExpression.create(`${documentMetadata}[2]['a:AttributeValue']['a:Value']`),
				'Incoming'
			)
			.addJsonValueExpectation(
				JsonPathExpression.create(`${documentMetadata}[3]['a:AttributeValue']['__i:type']`),
				'a:DateAttributeValue'
			)
			.addJsonValueExpectation(
				JsonPathExpression.create(`${documentMetadata}[3]['a:AttributeValue']['a:Name']`),
				'Document:Received/Sent Date'
			)
			.addJsonValueExpectation(
				JsonPathExpression.create(`${documentMetadata}[3]['a:AttributeValue']['a:Value']`),
				/.+/
			); // ISO 8601 strings are apparently really hard to validate, so just check that we have something!

		if (validateDocumentGroupType) {
			// Values for this are likely to change, so just check that we have something
			interaction.addJsonValueExpectation(
				JsonPathExpression.create(`${documentMetadata}[1]['a:AttributeValue']['a:Value']`),
				/.+/
			);
		}
