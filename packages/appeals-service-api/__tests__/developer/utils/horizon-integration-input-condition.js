...
class HorizonIntegrationInputCondition {
    ...
    get({
        ...
        expectedCaseworkReason = appeal.appealType == '1001' ? null : '4. Granted planning permission for the development subject to conditions to which you object'
    } = {}) {

        const lpaExpectations = {
            code: lpaCode,
            country: lpaCode == 'W69999999' ? 'Wales' : 'England',
            name: lpaCode == 'W69999999' ? 'System Test Borough Council Wales' : 'System Test Borough Council England',
            ...
            expectations: {
                createOrganisationInHorizonRequests: [
                    new HorizonCreateOrganisationRequestBodyExpectation(
                        appeal.appealType == '1001' ? {"__i:nil": "true"} : appeal.contactDetailsSection.contact.companyName
                    )
                ],
                createContactInHorizonRequests: [ 
                    new HorizonCreateContactRequestBodyExpectation('test@pins.com', 'Appellant', 'Name', `O_0`)	
                ],
        ...
        const agentIsSpecifiedInAppeal = appeal.appealType == '1001' ? 
        ...
        if (agentIsSpecifiedInAppeal) { 
            ...
            if (appeal.appealType == '1005') {
                ...
                condition.expectations.createOrganisationInHorizonRequests[0] = new HorizonCreateOrganisationRequestBodyExpectation(appeal.contactDetailsSection.appealingOnBehalfOf.companyName);
                condition.expectations.createOrganisationInHorizonRequests[1] = new HorizonCreateOrganisationRequestBodyExpectation(appeal.contactDetailsSection.contact.companyName);
            }

            // The first create contact request will be for the appellant, but their email isn't collected now
            condition.expectations.createContactInHorizonRequests[0] = new HorizonCreateContactRequestBodyExpectation({"__i:nil": "true"}, 'Appellant', 'Name', `O_0`)	
            
            // The second create contact request will be for the ahent, and their email is collected
            condition.expectations.createContactInHorizonRequests.push(new HorizonCreateContactRequestBodyExpectation('test@pins.com', 'Agent', 'Name', 'O_1'));

            // There'll now be another contact added to the create appeal request
            createAppealRequestContacts.push(new HorizonCreateAppealContactExpectation('P_1', 'Agent Name', 'Agent'))
        ...
        condition.expectations.createAppealInHorizonRequest = new HorizonCreateAppealRequestBodyExpectation(
        ...
        const dateRegex = new RegExp(/\d{2} (January|February|March|April|May|June|July|August|SeptemberOctober|November|December) \d{4}/)
        if (appeal.appealType == '1001') {
            ...
            condition.expectations.emailToLpa.templateVariables.push({'loca planning department': lpaExpectations.name})
            ...
            condition.expectations.emailToLpa.templateVariables.push({'non-determination': new RegExp(/yes|no/)})
        }
