 ...
 */
const setupOidc = (dependencies) => {
	const { primsaClient, logger } = dependencies;
	Adapter.setDependencies(primsaClient, logger);
	Account.setDependencies(dependencies);
	...
	oidc.registerGrantType(consts.AUTH.GRANT_TYPE.OTP, otp.handler(dependencies), otp.parameters, []);
	oidc.registerGrantType(
		consts.AUTH.GRANT_TYPE.ROPC,
		ropc.handler(dependencies),
		ropc.parameters,
		[]
	);
