...
exports.isFeatureActive =
	({ endpoint, timeToLiveInMinutes }) =>
	async (featureFlagName, localPlanningAuthorityCode) => {
		...
		if (featureFlagCache[flagName] === undefined || timeToLiveEvaluation) {
			...
			try {
				...
				if (azureFeatureFlagConfiguration && typeof azureFeatureFlagConfiguration === 'object') {
					...
					featureFlagCache[flagName].timeToLive = Date.now() + Number(timeToLiveInMinutes) * 60000;
				} else {
		...
		const userGroup =
			featureFlagConfiguration.conditions.client_filters[0].parameters.Audience.Users;
