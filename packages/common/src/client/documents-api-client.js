...
class DocumentsApiClient {
	...
	/**
	 * @param {string} blobUrl url or id figure this out
	 * @returns {Promise<any>}
	 */ // eslint-disable-next-line no-unused-vars
	async getBODocumentSASUrl(blobUrl) {
	...
	async handler(path, method = 'GET', opts = {}, headers = {}) {
		//todo: can we reuse this easiest is to extend class
	...
	#addAuthHeaders(headers) {
		...
		} else if (this.tokens.clientCreds) {
			headers.Authorization = 'Bearer ' + this.tokens.clientCreds;
		}
		if (this.tokens.id_token) {
			headers.Authentication = 'Bearer ' + this.tokens.id_token;
		}
	...
	#makeGetRequest(endpoint) {
		return this.handler(endpoint, 'GET', undefined, undefined);
	}
}
