...
Given(
	"an Appellant or Agent is on the 'Do you have any new plans for drawings that support your appeal' page",
	() => {
...
Then(
	"they are presented with the page 'Do you want to submit any new supporting documents with your appeal?'",
	() => {
...
Then('they will have to upload any valid files again', () => {
	cy.reload();
	getFileUploadButton().attachFile([validFilename2, validFilename2], {
		subjectType: 'drag-n-drop'
	});
	getSaveAndContinueButton().click();
...
When(
	'InvalidFile1 is replaced with {string} and uploaded along with the {string}',
	(validFile, invalidFile2) => {
		cy.reload();
