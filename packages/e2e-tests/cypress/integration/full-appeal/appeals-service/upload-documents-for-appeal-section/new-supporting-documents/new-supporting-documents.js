...
Given(
	"an Appellant or Agent is on the 'Do you want to submit any new supporting documents with your appeal?' page",
	() => {
		appealDocumentsSectionLink().click();
		cy.url().should('contain', appealStatementUrl);
...
Then(
	"the 'New supporting documents' page is displayed and the sub heading 'Drag and drop or choose files'",
	() => {
		cy.url().should('contain', Url);
...
When(
	"they upload a valid file through 'Choose files' or 'drag and drop' and click 'Continue'",
	() => {
...
Given(
	"an Appellant or Agent has uploaded a file {string} in the 'New supporting documents' page",
	(filename) => {
		newSupportingDocsMethods();
		if (filename === '') {
			getSaveAndContinueButton().click();
...
When(
	"they upload one {string} and one {string} through 'Choose files' or 'drag and drop' and click 'Continue'",
...
Then(
	"they are presented with the 'Do you want to submit any new supporting documents with your appeal?' page",
	() => {
		cy.url().should('contain', supportingDocumentsUrl);
	}
);
...
Given(
	"they can see the previously uploaded files and the text 'Replace the files' should be displayed",
	() => {
		getFileUploadButton().attachFile([validFilename1, validFilename2], {
			subjectType: 'drag-n-drop'
		});
		...
		cy.downloadFile(
			`${Cypress.env('APPEALS_BASE_URL')}/${Url}`,
			'cypress/downloads',
			validFilename2
		);
