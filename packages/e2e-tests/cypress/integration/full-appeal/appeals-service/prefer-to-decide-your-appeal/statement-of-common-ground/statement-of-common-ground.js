...
const pageTitle =
	'Upload your draft statement of common ground - Appeal a planning decision - GOV.UK';
...
Given(
	"the Appellant or Agent is on the 'How many days would you expect the inquiry to last?' page",
	() => {
		cy.url().should('contain', taskListUrl);
		linkDecideYourAppeal().click();
		cy.url().should('include', decideAppealUrl);
...
Given(
	"the Appellant or Agent is on the 'Upload your draft statement of common ground' page for {string}",
	(decideAppeal) => {
		switch (decideAppeal) {
			case 'Hearing':
				cy.url().should('contain', taskListUrl);
...
Given(
	"the Appellant or Agent has uploaded {string} as {string} in the 'Upload your draft statement of common ground' page",
	(filetype, fileName) => {
		if (fileName === '') {
			cy.url().should('contain', taskListUrl);
			linkDecideYourAppeal().click();
...
Then(
	"they are on the 'Upload your draft statement of common ground' page for {string} and can see the uploaded file",
	(decideAppeal) => {
		switch (decideAppeal) {
			case 'Hearing':
				cy.url().should('include', Url);
