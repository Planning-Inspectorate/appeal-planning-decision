...
const pageTitleVisibleFromRoad =
	'Is the site visible from a public road? - Appeal a planning decision - GOV.UK';
...
const visibleFromRoadDetailsErrorTextBox =
...
Given(
	"an appellant or agent is on the 'Is the appeal site part of an agricultural holding?' page",
	() => {
...
When(
	"the user selects No and Enter more than 255 characters in the text box and clicks 'Continue'",
	() => {
		const count = 255;
...
When(
	"the user selects 'No' and enters details about how the visibility is restricted and clicks 'Continue'",
	() => {
		selectNo().click();
...
Then(
	"the user is taken to the next page 'Are there any health and safety issues on the appeal site?'",
	() => {
...
Then('they are presented with an error message {string}', (errorMessage) => {
	switch (errorMessage) {
		...
		case visibleFromRoadDetailsErrorTextBox:
			verifyErrorMessage(
				visibleFromRoadDetailsErrorTextBox,
				errorMessageVisibleFromRoadDetails,
				getErrorMessageSummary
			);
			break;
...
Then(
	"they are presented with the 'Is the appeal site part of an agricultural holding?' page",
	() => {
		cy.url().should('contain', agriculturalHoldingUrl);
	}
);
