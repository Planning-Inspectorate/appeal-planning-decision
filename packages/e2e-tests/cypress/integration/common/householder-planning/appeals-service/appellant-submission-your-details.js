...
And('appeal is not updated with the original applicant {string}', () => {
	goToAppealsPage(pageURLAppeal.goToApplicantNamePage);
	confirmOriginalApplicantName('');
});

And(
	'the user can see that their appeal {string} been updated to show that they are acting on behalf of {string}',
	(hasOrHasNot, originalApplicant) => {
		goToAppealsPage(pageURLAppeal.goToApplicantNamePage);
		if (hasOrHasNot === 'has') {
			confirmOriginalApplicantName(originalApplicant);
		} else {
...
Then(
	'the user is informed that the provided value {string} is invalid because {string}',
	(nameOrEmail, reason) => {
		switch (reason) {
			case 'name missing':
				confirmDetailsWasRejected('Enter your name');
...
Then(
	'original applicant value {string} is invalid because {string}',
	(originalApplicant, reason) => {
		switch (reason) {
			case 'name missing':
...
Then('appeal is updated with new valid name and email', () => {
	goToAppealsPage(pageURLAppeal.goToYourDetailsPage);
	confirmNameValue('New Valid Name');
	confirmEmailValue('new-valid@email.com');
});

Then('appeal is updated with {string} and {string}', (name, email) => {
	goToAppealsPage(pageURLAppeal.goToYourDetailsPage);
...
Given(
	'the user {string} previously stated {string} the original appellant',
	(alreadySubmitted, originalAppellant) => {
		goToAppealsPage(pageURLAppeal.goToWhoAreYouPage);
...
Then('appeal is not updated because confirmation of original applicant status is required', () => {
	confirmWhoAreYouRejectedBecause(
		'Select yes if the original planning application was made in your name'
	);
