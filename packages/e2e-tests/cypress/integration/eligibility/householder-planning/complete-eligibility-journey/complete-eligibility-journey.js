...
import { getDateHouseholderDecisionReceived } from '../../../../support/eligibility/date-decision-received/get-date-householder-decision-received';
import {
  getBackLink,
...
Given(
  'appellant enters the date within {string} when the {string} was received',
  (deadline_duration, application_decision) => {
...
Given(
  "appellant selects the option yes for 'Are the conditions for householder planning permission?'",
  () => {
    verifyPageTitle(
      'Are the conditions for householder planning permission? - Before you start - Appeal a planning decision - GOV.UK',
    );
...
Then(
  'data is persisted for the date within {string} when the {string} was received',
  (deadline_duration, application_decision) => {
    if (deadline_duration === '6 months' && application_decision === 'Granted') {
      validDate = getPastDate(allowedDatePart.MONTH, 3);
      verifyPageTitle(
...
Then(
  "data is persisted for option yes for 'Are the conditions for householder planning permission?'",
  () => {
    verifyPageTitle(
      'Are the conditions for householder planning permission? - Before you start - Appeal a planning decision - GOV.UK',
    );
