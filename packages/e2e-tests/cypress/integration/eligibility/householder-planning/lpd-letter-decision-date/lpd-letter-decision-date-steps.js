...
import { verifyHouseholderDecisionDatesHighlights } from '../../../../support/eligibility/date-decision-received/enter-date-householder-decision-received';
...
Given(
	'appellant is on the what date was the decision received page for {string}',
...
When(
	'appellant enters the date lesser than the deadline date for {string}',
	(application_decision) => {
		if (application_decision === 'Granted') {
...
When(
	'appellant enters future date decision received of {string}-{string} for {string}',
	(datePart, value, applicationDecision) => {
		const futureDate = getFutureDate(datePart, value);
		if (applicationDecision === 'Granted')
			enterDateDecisionReceived({
				day: ('0' + getDate(futureDate)).slice(-2),
				month: ('0' + (getMonth(futureDate) + 1)).slice(-2),
...
When(
	'appellant enters the date older than the deadline date for {string}',
	(application_decision) => {
		if (application_decision === 'Granted') {
			const pastDate = getPastDate(allowedDatePart.MONTH, 7);
			enterDateDecisionReceived({
				day: ('0' + getDate(pastDate)).slice(-2),
				month: ('0' + (getMonth(pastDate) + 1)).slice(-2),
...
When(
	'appellant enters date decision received of {string}-{string}-{string}',
	(day, month, year) => {
...
When(
	'appellant enters date decision received of {string}-{string}-{string} for {string}',
	(day, month, year, applicationDecision) => {
		if (applicationDecision === 'Granted') enterDateDecisionReceived({ day, month, year });
		else enterDateHouseholderDecisionReceived({ day, month, year });
	}
);

When(
	'appellant is navigated to the granted or refused page for {string}',
	(application_decision) => {
...
Then(
	'decision received date they have inputted will not be saved for {string}',
	(applicationDecision) => {
		if (applicationDecision === 'Granted') getDateDecisionReceivedDay().should('have.text', '');
		else getDateHouseholderDecisionReceivedDay().should('have.text', '');
	}
);
...
Then(
	'appellant gets routed to a page which notifies them that the decision appeal date has passed',
	() => {
...
Then(
	'progress is halted with an error: {string} for {string}',
	(errorMessage, applicationDecision) => {
		if (applicationDecision === 'Granted')
			verifyErrorMessage(errorMessage, getPlanningApplicationDecisionError, getErrorMessageSummary);
		else
			verifyErrorMessage(
				errorMessage,
				getPlanningApplicationHouseholderDecisionError,
				getErrorMessageSummary
			);
	}
);
...
Then(
	'the correct input {string} is highlighted for {string}',
	(highlights, applicationDecision) => {
		if (applicationDecision === 'Granted') verifyDecisionDatesHighlights(highlights);
		else verifyHouseholderDecisionDatesHighlights(highlights);
	}
);
