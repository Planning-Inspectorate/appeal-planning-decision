...
const getAppealTypeFeedbackUrl = (appealTypeCode) => {
	switch (appealTypeCode) {
		...
		case 'ENFORCEMENT_LISTED':
			return feedbackUrlEnforcementListed;
		default:
			return feedbackUrl;
	}
};
...
const feedbackUrlEnforcementListed =
	process.env.FEEDBACK_URL_ENFORCEMENT_LISTED ||
...
module.exports = {
	...
	fileUpload: {
		...
		pins: {
			...
			maxFileUploadSizeEnvironmentStatement: numberWithDefault(
				process.env.FILE_UPLOAD_MAX_FILE_SIZE_BYTES_ENVIRONMENT_STATEMENT,
				oneGigabyte
			),
			allowedFileTypes: {
