...
const mockResponse = {
	journeyId: JOURNEY_TYPES.S78_QUESTIONNAIRE_PART_1.id,
	LPACode: 'Q9999',
	referenceId: '123',
	answers: {}
};

describe('S78 Part 1 Journey', () => {
	...
	it('should define sections and questions', () => {
		...
		expect(journey.sections[6].segment).toBe('appeal-process');
		expect(journey.sections[6].questions.map((q) => q.fieldName)).toContain('newConditions');
		expect(journey.sections[6].questions.map((q) => q.fieldName)).toContain(
