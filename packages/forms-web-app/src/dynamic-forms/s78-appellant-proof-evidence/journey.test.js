...
const { baseAppellantProofEvidenceUrl, ...params } = require('./journey');
...
describe('S78 Final Comments Journey class', () => {
	describe('constructor', () => {
		...
		it('should set baseUrl', () => {
			...
			expect(journey.baseUrl).toEqual(expect.stringContaining(baseAppellantProofEvidenceUrl));
			expect(journey.baseUrl).toEqual(expect.stringContaining(mockResponse.referenceId));
		});

		it('should set taskListUrl', () => {
			const journey = new Journey({ ...params, response: mockResponse });
			expect(journey.taskListUrl).toEqual('/appeals/proof-evidence/123');
		});
