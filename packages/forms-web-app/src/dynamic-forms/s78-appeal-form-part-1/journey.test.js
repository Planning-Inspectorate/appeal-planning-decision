...
describe('S78 Appeal Form Part 1 Journey', () => {
	...
	it('should structure upload documents properly', () => {
		const journey = new Journey({ ...params, response: mockResponse });
		const uploadSection = journey.sections.find((s) => s.segment === 'upload-documents');

		const submitEnvStatement = uploadSection.questions.find(
			(q) => q.fieldName === 'appellantSubmittedEnvironmentalStatement'
		);
		const uploadEnvStatement = uploadSection.questions.find(
			(q) => q.fieldName === 'uploadEnvironmentalStatement'
		);

		expect(submitEnvStatement).toBeDefined();
		expect(uploadEnvStatement).toBeDefined();
	});
});
