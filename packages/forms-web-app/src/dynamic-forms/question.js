...
class Question {
	...
	async saveResponseToDB(apiClient, journeyResponse, responseToSave) {
		...
		const saveMethodMap = {
			[`${JOURNEY_TYPE.questionnaire}_${LPA_USER_ROLE}`]: apiClient.patchLPAQuestionnaire,
			[`${JOURNEY_TYPE.appealForm}_${APPELLANT}`]: apiClient.updateAppellantSubmission,
			[`${JOURNEY_TYPE.statement}_${LPA_USER_ROLE}`]: apiClient.patchLPAStatement,
			[`${JOURNEY_TYPE.statement}_${RULE_6_PARTY}`]: apiClient.patchRule6StatementSubmission,
			[`${JOURNEY_TYPE.finalComments}_${APPELLANT}`]:
				apiClient.patchAppellantFinalCommentSubmission,
			[`${JOURNEY_TYPE.finalComments}_${LPA_USER_ROLE}`]: apiClient.patchLPAFinalCommentSubmission,
			[`${JOURNEY_TYPE.proofEvidence}_${APPELLANT}`]:
				apiClient.patchAppellantProofOfEvidenceSubmission,
			[`${JOURNEY_TYPE.proofEvidence}_${LPA_USER_ROLE}`]:
				apiClient.patchLpaProofOfEvidenceSubmission,
			[`${JOURNEY_TYPE.proofEvidence}_${RULE_6_PARTY}`]:
				apiClient.patchRule6ProofOfEvidenceSubmission
		};

		const save = saveMethodMap[key];
		if (!save) throw new Error(`No save function found for journey type: ${key}`);
		return await save(journeyResponse.referenceId, responseToSave.answers);
	}
