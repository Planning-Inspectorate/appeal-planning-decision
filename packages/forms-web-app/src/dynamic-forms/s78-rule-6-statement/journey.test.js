...
const { baseRule6StatementUrl, ...params } = require('./journey');
...
describe('S78 Rule6 Statement Journey class', () => {
	describe('constructor', () => {
		...
		it('should set baseUrl', () => {
			...
			expect(journey.baseUrl).toEqual(expect.stringContaining(baseRule6StatementUrl));
			expect(journey.baseUrl).toEqual(expect.stringContaining(mockResponse.referenceId));
		});

		it('should set taskListUrl', () => {
			const journey = new Journey({ ...params, response: mockResponse });
			expect(journey.taskListUrl).toEqual('/rule-6/appeal-statement/123');
		});

		it('should set template', () => {
			const journey = new Journey({ ...params, response: mockResponse });
			expect(journey.journeyTemplate).toBe('rule-6-statement-template.njk');
		});
