...
describe('controllers/full-appeal/listed-building', () => {
	...
	describe('postListedBuilding', () => {
		...
		describe('if type-of-planning-application is not LDC', () => {
			...
			it(`should set appeal type to HAS and redirect to the granted-or-refused-householder page if HAS and 'no' is selected`, async () => {
				...
				expect(res.redirect).toHaveBeenCalledWith(
					'/before-you-start/granted-or-refused-householder'
				);
			});
		...
		describe('if type-of-planning-application is LDC', () => {
			it(`should retain appeal type as LDC and redirect to granted-or-refused page if 'yes' is selected`, async () => {
				req = mockReq(fullAppeal);
				...
				mockRequest.session.appeal.appealType = APPEAL_ID.LAWFUL_DEVELOPMENT_CERTIFICATE;
				mockRequest.session.appeal.typeOfPlanningApplication = LAWFUL_DEVELOPMENT_CERTIFICATE;

				await postListedBuilding(mockRequest, res);

				expect(fullAppeal.eligibility.isListedBuilding).toEqual(true);
				expect(fullAppeal.appealType).toEqual(APPEAL_ID.LAWFUL_DEVELOPMENT_CERTIFICATE);
				expect(createOrUpdateAppeal).toHaveBeenCalledWith({
					...fullAppeal,
					appealType: APPEAL_ID.LAWFUL_DEVELOPMENT_CERTIFICATE,
					eligibility: { ...fullAppeal.eligibility, isListedBuilding: true }
				});

				expect(res.redirect).toHaveBeenCalledWith('/before-you-start/granted-or-refused');
			});

			it(`should retain appeal type as LDC and redirect to can-use-service if 'no' is selected`, async () => {
				req = mockReq(fullAppeal);
