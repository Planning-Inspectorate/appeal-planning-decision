...
Given(
	'LPA Planning Officer has not added any data to the Representations from Interested Parties question',
	() => {
...
When(
	'LPA Planning Officer chooses to upload the document Representations from interested parties',
	() => {
...
Then(
	'LPA Planning Officer is presented with the ability to upload any documents relevant to the question Representations from interested parties',
	() => {
		verifyPage(page.url);
...
Then(
	'Representations from Interested parties heading and the uploaded file name should be displayed',
	() => {
...
Then('the status is not started', () => {
	cy.get(`li[${page.id}-status="NOT STARTED"]`).find('.govuk-tag').contains('NOT STARTED');
});
