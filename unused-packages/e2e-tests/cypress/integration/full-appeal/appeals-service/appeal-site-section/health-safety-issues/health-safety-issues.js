...
const pageTitleHealthSafetyIssues =
	'Are there any health and safety issues on the appeal site? - Appeal a planning decision - GOV.UK';
...
When(
	"the user selects 'Yes' and enters details about the health and safety issues and clicks 'Continue'",
	() => {
		selectYes().click();
		healthAndSafetyIssuesProvideDetails().type(
			`{selectall}{backspace}The site has no mobile reception`
		);
		getSaveAndContinueButton().click();
	}
);
...
Given(
	"an appellant or agent is on the 'Are there any health and safety issues on the appeal site?' page",
	() => {
		aboutAppealSiteSectionLink().click();
		cy.url().should('contain', siteAddressUrl);
		provideAddressLine1(addressLine1);
...
Then('they are presented with an error message {string}', (errorMessage) => {
	switch (errorMessage) {
		case healthSafetyIssuesDetailsError:
			verifyErrorMessage(
				healthSafetyIssuesDetailsError,
				errorMessageHealthSafetyIssuesDetails,
				getErrorMessageSummary
			);
			break;
