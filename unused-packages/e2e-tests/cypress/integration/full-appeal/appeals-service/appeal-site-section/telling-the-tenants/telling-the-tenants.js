...
const pageTitleTellingTheOtherTenants =
	'Telling the other tenants - Appeal a planning decision - GOV.UK';
...
When(
	'the user select the confirmation boxes for the {string} and click continue',
	(checkboxOptions) => {
		let tenantsCheckboxValues = checkboxOptions.split(',');
		tenantsCheckboxValues.forEach((ele) => {
			if (
				ele.trim() === "I've told all the tenants about my appeal" ||
				ele.trim() === "I've told all the other tenants about my appeal"
			) {
...
When(
	'the user does not select the any one of the confirmation checkboxes boxes then an error message {string} is displayed',
	(errorMessage) => {
		getSaveAndContinueButton().click();
		verifyErrorMessage(errorMessage, errorMessageTellingTheTenants, getErrorMessageSummary);
