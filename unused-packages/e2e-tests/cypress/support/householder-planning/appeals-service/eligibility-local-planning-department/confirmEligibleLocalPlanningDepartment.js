	...
	cy.get('[data-cy="eligible-departments"]')
		...
		.then((departments) => {
			...
			if (eligiblePlanningDepartments.length > 0) {
				lastEligibleLocalPlanningDepartment =
					eligiblePlanningDepartments[eligiblePlanningDepartments.length - 1];
			}

			goToAppealsPage(pageURLAppeal.goToPlanningDepartmentPage);
			cy.get('input#local-planning-department').should(
				'have.value',
				lastEligibleLocalPlanningDepartment
			);

			goToAppealsPage(pageURLAppeal.goToCheckYourAnswersPage);
			cy.get('[data-cy=local-planning-department]')
				.first()
				.should('contain', lastEligibleLocalPlanningDepartment);
		});
};
