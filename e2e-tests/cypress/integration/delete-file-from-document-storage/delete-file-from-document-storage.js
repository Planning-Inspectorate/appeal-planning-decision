...
Given('a request is made to delete a file', async () => {
    ...
    let [err, res] = await cy.request('POST', 'http://localhost:3001/api/v1/${applicationId}', documentData)
                                .then((data) => [null, data])
                                .catch((err)=>[err,null])
    ...
    /*
    [err, res] = await cy.request('DELETE', 'http://localhost:3001/api/v1/${documentId}')
                                .then((data) => [null, data])
                                .catch((err)=>[err,null])
    expect(err).toBe(null)
    */
