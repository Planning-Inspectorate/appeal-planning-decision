...
export const commercialadvertQuestionnaire = (context, lpaManageAppealsData, lpaAppealType, caseRef = '') => {
	...
	let appealId = '';
	const targetAppealType = lpaAppealType || lpaManageAppealsData?.commercialadvAppealType; // fallback
	...
	selectAppealRow().then(() => {
		...
		// Verify appeal type shown matches expectation (commercialAdv or provided type)
		cy.get(basePage?._selectors.dlgovukSummaryListAppealDetails).within(() => {
			cy.get(basePage?._selectors.govukSummaryListRow).each(($row) => {
				...
				if ($key.text().trim() === lpaManageAppealsData?.appealType) {
					// Determine expected value: commercialadvert, or fallback, or passed type
					const expected = targetAppealType === lpaManageAppealsData?.commercialadvAppealType
						? lpaManageAppealsData?.appealTypeCommercialAdvert
						: (lpaManageAppealsData?.appelTypeAdvert || targetAppealType);
		...
		const correctTypeText = targetAppealType === lpaManageAppealsData?.commercialadvAppealType
		...
		// Constraints & designations subset needed for commercialadv / adv
		constraintsAndDesignations.selectCorrectTypeOfAppeal(context);
		constraintsAndDesignations.selectChangesListedBuilding(context);
		...
		constraintsAndDesignations.selectAreaOfOutstandingNaturalBeauty(context);
		...
		// Planning officer's report & supporting docs 
		poReportAndSupportDocs.selectPOReportAndSupportDocsAdv(context, lpaManageAppealsData, targetAppealType);
		...
		poReportAndSupportDocs.selectEmergingPlansAdvert(context);
		...
		appealProcess.selectNearbyAppeals(context, lpaManageAppealsData, lpaManageAppealsData?.commercialadvAppealType);
