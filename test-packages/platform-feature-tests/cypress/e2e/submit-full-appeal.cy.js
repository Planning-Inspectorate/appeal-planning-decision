	...
	submitFullAppealData.forEach((context) => {
		const {
			...
			//typeOfPlanningApplication?.forEach((planning) => {
				...
				application form has area unit ${applicationForm?.areaUnits}  and know other land owners ${applicationForm?.knowsOtherOwners ?? applicationForm?.knowsAllOwners} final comments config is: ${finalComments}`, () => {
				submitAppealFlow({
					statusOfOriginalApplication,
					typeOfDecisionRequested,
					statusOfPlanningObligation,
