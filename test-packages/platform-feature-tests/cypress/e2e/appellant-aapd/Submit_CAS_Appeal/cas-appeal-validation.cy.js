...
describe('CAS Apeal Date Validations', () => {
    ...
    it(`Validate future date error message  in decision date page future month`, () => {
        cy.get(prepareAppealSelector?._minorCommercialDevelopmentSelectors?.decisionDateDay).type(date.today());
        cy.get(prepareAppealSelector?._minorCommercialDevelopmentSelectors?.decisionDateMonth).type(date.nextMonth());
        const currentYear = date.currentMonth() > date.nextMonth() ? date.nextYear() : date.currentYear();
        cy.get(prepareAppealSelector?._minorCommercialDevelopmentSelectors?.decisionDateYear).type(currentYear);
        cy.advanceToNextPage();
    ...
    it(`Validate future date error message  in decision date page future day`, () => {
        cy.get(prepareAppealSelector?._minorCommercialDevelopmentSelectors?.decisionDateDay).type(date.nextDay());
        cy.get(prepareAppealSelector?._minorCommercialDevelopmentSelectors?.decisionDateMonth).type(date.currentMonth());
        cy.get(prepareAppealSelector?._minorCommercialDevelopmentSelectors?.decisionDateYear).type(date.currentYear());
        cy.advanceToNextPage();
    ...
    it(`Validate future date error message  in decision date page negative date`, () => {
        cy.get(prepareAppealSelector?.__minorCommercialDevelopmentSelectors?.decisionDateDay).type(-1);
        cy.get(prepareAppealSelector?._minorCommercialDevelopmentSelectors?.decisionDateMonth).type(date.currentMonth());
        cy.get(prepareAppealSelector?._minorCommercialDevelopmentSelectors?.decisionDateYear).type(date.currentYear());
        cy.advanceToNextPage();
    ...
    it(`Validate future date error message  in decision date page past year`, () => {
        cy.get(prepareAppealSelector?.__minorCommercialDevelopmentSelectors?.decisionDateDay).type(date.today());
        cy.get(prepareAppealSelector?._minorCommercialDevelopmentSelectors?.decisionDateMonth).type(date.currentMonth());
        cy.get(prepareAppealSelector?._minorCommercialDevelopmentSelectors?.decisionDateYear).type(date.previousYear());
        cy.advanceToNextPage();
...
describe('MInor Commercial Validations for enforcement', () => {
    const prepareAppealSelector = new PrepareAppealSelector();
    const basePage = new BasePage();   
...
describe('Minor Commercial Validations', () => {
    const prepareAppealSelector = new PrepareAppealSelector();
    const basePage = new BasePage();
    ...
    const context = casAppealRefusedTestCases[0];
    ...
    beforeEach(() => {
        ...
        cy.getByData(basePage?._selectors?.answerCASAppeal).click();
        cy.advanceToNextPage();

        cy.getByData(basePage?._selectors?.aboutSelection).click();//none of these selection for CAS appeal
        cy.advanceToNextPage();
    ...
    it(`Validate data entered while adding the prepare appeal form `, () => {
        ...
        cy.location('search').then((search) => {
            ...
            cy.advanceToNextPage();
            //What is the application reference number?minorCommercialDevelopmentApplicationType
