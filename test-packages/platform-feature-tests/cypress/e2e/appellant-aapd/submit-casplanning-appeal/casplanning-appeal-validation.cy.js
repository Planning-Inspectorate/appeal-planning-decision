...
describe('CAS Apeal Date Validations', () => {
    ...
    beforeEach(() => {
        ...
        cy.getByData(basePage?._selectors?.answerMinorCommercialDevelopment).click();                              
        cy.advanceToNextPage();

        cy.getByData(basePage?._selectors?.appealSelection6).click();
        cy.advanceToNextPage();
    ...
    it(`Validate future date error message  in decision date page future month`, () => {
        cy.get(prepareAppealSelector?._casPlanningSelectors?.decisionDateDay).type(date.today());
        cy.get(prepareAppealSelector?._casPlanningSelectors?.decisionDateMonth).type(date.nextMonth());
        const currentYear = date.currentMonth() > date.nextMonth() ? date.nextYear() : date.currentYear();
        cy.get(prepareAppealSelector?._casPlanningSelectors?.decisionDateYear).type(currentYear);
        cy.advanceToNextPage();
    ...
    it(`Validate future date error message  in decision date page future day`, () => {
        cy.get(prepareAppealSelector?._casPlanningSelectors?.decisionDateDay).type(date.nextDay());
        cy.get(prepareAppealSelector?._casPlanningSelectors?.decisionDateMonth).type(date.currentMonth());
        cy.get(prepareAppealSelector?._casPlanningSelectors?.decisionDateYear).type(date.currentYear());
        cy.advanceToNextPage();
    ...
    it(`Validate future date error message  in decision date page negative date`, () => {
        cy.get(prepareAppealSelector?._casPlanningSelectors?.decisionDateDay).type(-1);
        cy.get(prepareAppealSelector?._casPlanningSelectors?.decisionDateMonth).type(date.currentMonth());
        cy.get(prepareAppealSelector?._casPlanningSelectors?.decisionDateYear).type(date.currentYear());
        cy.advanceToNextPage();
    ...
    it(`Validate future date error message  in decision date page past year`, () => {
        cy.get(prepareAppealSelector?._casPlanningSelectors?.decisionDateDay).type(date.today());
        cy.get(prepareAppealSelector?._casPlanningSelectors?.decisionDateMonth).type(date.currentMonth());
        cy.get(prepareAppealSelector?._casPlanningSelectors?.decisionDateYear).type(date.previousYear());
        cy.advanceToNextPage();
...
describe('CAS Planning Validations for enforcement', () => {
    const prepareAppealSelector = new PrepareAppealSelector();
    const basePage = new BasePage();   
...
describe('CAS Planning Validations', () => {
    const prepareAppealSelector = new PrepareAppealSelector();
    const basePage = new BasePage();
    ...
    const ownSomeLandPage = new OwnSomeLandPage();0
    ...
    const context = casPlanningAppealRefusedTestCases[0];
    ...
    it(`Validate change URL for application name in task link page `, () => {
        ...
        cy.location('search').then((search) => {
            ...
            cy.get(`a[href*="/appeals/cas-planning/prepare-appeal/application-name?id=${dynamicId}"]`).contains('Change')
        });
    });
