...
import { advertAppealNoDecisionTestCases } from "cypress/helpers/appellantAAPD/advertAppeal/advertAppealNoDecisionData";
...
const{HighwayLandPage}=require("../../../support/flows/pages/appellant-aapd/prepare-appeal/highwayLandPage");
const{AdvertisementPositionPage}=require("../../../support/flows/pages/appellant-aapd/prepare-appeal/advertisementPositionPage");
...
const{DescriptionDevelopmentCorrectPage}=require("../../../support/flows/pages/appellant-aapd/prepare-appeal/descriptionDevelopmentCorrectPage");
...
describe('Advert Appeal Validations for enforcement',{ tags:'@advert-appeal-validation-1' }, () => {
...
describe('Advert Appeal Validations',{ tags:'@advert-appeal-validation-2' }, () => {
    const prepareAppealSelector = new PrepareAppealSelector();
    const basePage = new BasePage();
   ...
   // const siteAreaPage = new SiteAreaPage();
    ...
    const context = advertAppealGrantedTestCases[0];
    const decideAppealsPage = new DecideAppealsPage();
    const descriptionDevelopmentCorrectPage=new DescriptionDevelopmentCorrectPage();
   // const agriculturalHoldingPage = new AgriculturalHoldingPage();
    ...
    beforeEach(() => {
        ...
        cy.getByData(basePage?._selectors?.answerMinorCommercialAdvertisment).click();
        cy.advanceToNextPage();

        cy.getByData(basePage?._selectors?.answerGranted).click();
        cy.advanceToNextPage();
    ...
    it(`Validate change URL for application name in task link page `, () => {
        ...
        cy.location('search').then((search) => {
            ...
            cy.get(`a[href*="/appeals/adverts/prepare-appeal/application-name?id=${dynamicId}"]`).contains('Change')
        });
    });
    ...
    it(`Validate data entered while adding the prepare appeal form `, () => {
        ...
        cy.location('search').then((search) => {
           ...
           // siteAreaPage.addSiteAreaData(context?.typeOfDecisionRequested, context?.applicationForm?.areaUnits, context, prepareAppealData);
            ...
            //Does the appeal site include any highway land?
            highwayLandPage.selectHighwayLand(context?.applicationForm?.isHighwayLand, context);

            //Where is the advertisement to be positioned on the site?
