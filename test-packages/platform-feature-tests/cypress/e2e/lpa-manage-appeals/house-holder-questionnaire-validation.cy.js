...
describe('House Holder appleal questionnaire validation', () => {
	...
	beforeEach(() => {
		...
		cy.get(basePage?._selectors.trgovukTableRow).each(($row) => {
			...
			if (
				rowtext.includes(lpaManageAppealsData?.hasAppealType) &&
				!rowtext.includes(lpaManageAppealsData?.todoInvalid)
			) {
				if (counter === 0) {
					cy.wrap($row).within(() => {
						cy.get(basePage?._selectors.trgovukTableCell)
							.contains(lpaManageAppealsData?.hasAppealType)
							.should('be.visible');
						cy.get('a').each(($link) => {
		...
		cy.get(basePage?._selectors.govukSummaryListKey)
			...
			.then(($link) => {
				...
				if (linkText === 'Upload') {
					...
					cy.get(basePage?._selectors.govukErrorSummaryList)
						.find('a')
						.should('have.attr', 'href', '#uploadPlansDrawings')
						.and('contain.text', 'Select the plans, drawings and list of plans');
				} else if (linkText === 'Change') {
					cy.get(basePage?._selectors.govukSummaryListKey)
						.contains('Upload the plans, drawings and list of plans')
						.closest(basePage?._selectors.govukSummaryListRow)
