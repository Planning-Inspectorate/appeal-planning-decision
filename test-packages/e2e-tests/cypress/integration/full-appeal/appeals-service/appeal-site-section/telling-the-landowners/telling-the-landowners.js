...
const pageTitleTellingTheLandowners =
	'Telling the landowners - Appeal a planning decision - GOV.UK';
const tellingTheLandownersCaption = 'Tell us about the appeal site';
const pageTitleTellingTheOtherLandowners =
	'Telling the other landowners - Appeal a planning decision - GOV.UK';
const pageTitleAgriculturalLand =
	'Is the appeal site part of an agricultural holding? - Appeal a planning decision - GOV.UK';

Given(
	'an appellant or agent is on the Do you know who owns the land involved in the appeal page for {string}',
	(landowners) => {
...
Given(
	'an appellant or agent is on the Telling the landowners page for the {string} and {string}',
	...
	if (landowners === 'Telling the landowners') {
		...
		tellingTheLandownersFormInAnnex()
			.should(
				'have.attr',
				'href',
				'https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/994918/eHow_To_-_Planning_18_ANNEX.pdf'
			)
...
Then('they are presented with the {string} page', (page) => {
	if (page === 'know the owners') cy.url().should('contain', knowTheOwnersUrl);
	else if (page === 'Advertising your appeal') cy.url().should('contain', advertisingYourAppealUrl);
});
