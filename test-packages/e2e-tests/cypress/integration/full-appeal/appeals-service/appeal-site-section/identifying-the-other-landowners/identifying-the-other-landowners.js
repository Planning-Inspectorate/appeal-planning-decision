...
Given(
	"an Appellant or Agent is on the 'Do you know who owns the rest of the land involved in the Appeal' page",
	() => {
...
Then('the {string} page is displayed with guidance text', () => {
...
Then(
	"they are on the 'Do you know who owns the rest of the land involved in the Appeal' page and the option {string} is selected",
	(knowTheOwners) => {
		switch (knowTheOwners) {
			case 'I know who owns some of the land':
...
Given(
	'an Appellant or Agent is on the {string} page for the question {string}',
	(currentPage, knowTheOwners) => {
		switch (knowTheOwners) {
			case 'I know who owns some of the land':
				methodsKnowTheOwners();
				selectSomeOf().click();
...
Then('the user is taken to the {string}', () => {
	cy.url().should('include', advertisingYourAppealUrl);
});
When("When they click on the 'Back' link", () => {
	getBackLink().click();
});
Then('they are presented with the {string} page for the question {string}', () => {
	cy.url().should('include', url);
});
...
Then(
	"they are presented with the 'Do you know who owns the land involved in the Appeal' page",
	() => {
		cy.url().should('include', knowTheOwnersUrl);
	}
);
