Feature: Green belt
  ...
  Scenario: AC02 The LPA does not provide an answer to Green belt
    Given 'Green belt' question has been requested
    When an answer to the question is not provided
    Then progress is halted with an error message to select an answer

  Scenario: AC03 LPA Officer selects Yes to Green belt
    Given 'Green belt' question has been requested
    When the answer is 'yes'
    Then officer progresses to the task list from yes no question
    And the 'Green belt' subsection is shown as completed

  Scenario: AC04 LPA Officer selects No to Green belt
    Given 'Green belt' question has been requested
  ...
  Scenario: AC06 LPA Officer returns to the completed Green belt question
    Given The yes or no question 'Green belt' has been completed
  ...
  Scenario: AC08 Check Your Answers Page
    ...
    Then 'Green belt' yes or no question and answer should be displayed

  Scenario: AC09 Change answer from Check Your Answers Page
    Given the questionnaire has been completed
    And a change to answer 'Green belt' is requested from Change your answers page
