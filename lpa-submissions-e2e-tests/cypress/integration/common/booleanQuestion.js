...
const TEXT_ID = 'booleanInput-text';
...
Then('the updated yes or no answer is displayed', () => {
  cy.get('@page').then(({ id, textChildOf }) => {
    if (textChildOf) {
      cy.confirmCheckYourAnswersDisplayed(id, textChildOf ? NO_ID : YES_ID);
    } else {
...
Then('{string} yes or no answer is displayed on the PDF', () => {
  if (!Cypress.env('ASSUME_LIMITED_ACCESS')) {
    cy.get('@page').then(({ heading, textChildOf, textMock }) => {
      cy.downloadSubmissionPdf().then(() => {
        cy.checkSubmissionPdfContent(heading);

        if (textChildOf) {
          cy.checkSubmissionPdfContent(textChildOf);
          cy.checkSubmissionPdfContent(textMock);
        } else {
          cy.checkSubmissionPdfContent('Yes');
        }
      });
    });
  }
